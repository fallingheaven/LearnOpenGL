...
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| ![image-20251029191421690](assets\image-20251029191421690.png) | ![image-20251029191426097](assets\image-20251029191426097.png) |
...
<img src="assets\image-20251029191432246.png" alt="image-20251029191432246" style="zoom:50%;" /> 
...
<img src="assets\image-20251029191458351.png" alt="image-20251029191458351" style="zoom:50%;" /> 

不过这样做有一些限制，它只能做第一次照射物体的阴影，后面的阴影就会变淡；而且因为阴影贴图存储的是深度最浅的那个，导致后面的阴影都是以最浅的那个作为标准；而且使用正面剔除总会有点问题，总的来说只是一个特化的一个方法，没有普遍意义

![image-20251029191505283](assets\image-20251029191505283.png) 
...
| :----------------------------------------------------------: | :----------------------------------------------------------: | :----------------------------------------------------------: |
| ![image-20251029191524901](assets\image-20251029191524901.png) | ![image-20251029191530509](assets\image-20251029191530509.png) | ![image-20251029191534719](assets\image-20251029191534719.png) |
...
| ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| ![image-20251029191540212](assets\image-20251029191540212.png) | ![image-20251029191542306](assets\image-20251029191542306.png) | ![image-20251029191544841](assets\image-20251029191544841.png) | ![image-20251029191546734](assets\image-20251029191546734.png) |
...
|                           不加 HDR                           |                            加 HDR                            |
| :----------------------------------------------------------: | :----------------------------------------------------------: |
| ![image-20251029191552036](assets\image-20251029191552036.png) | ![image-20251029191555860](assets\image-20251029191555860.png) |
...
|                           延迟渲染                           |                           前向渲染                           |                      延迟渲染+前向渲染                       |
| :----------------------------------------------------------: | :----------------------------------------------------------: | :----------------------------------------------------------: |
| ![image-20251029191559125](assets\image-20251029191559125.png) | ![image-20251029191601183](assets\image-20251029191601183.png) | ![image-20251029191603001](assets\image-20251029191603001.png) |
...
| :----------------------------------------------------------: | ------------------------------------------------------------ | :----------------------------------------------------------: |
| ![image-20251029191610809](assets\image-20251029191610809.png) | ![image-20251029191612735](assets\image-20251029191612735.png) | ![image-20251029191614695](assets\image-20251029191614695.png) |
...
![image-20251027163118418](assets\image-20251027163118418.png) 
...
| :----------------------------------------------------------: | :----------------------------------------------------------: | :----------------------------------------------------------: |
| ![image-20251028130901665](assets\image-20251028130901665.png) | ![image-20251028130917781](assets\image-20251028130917781.png) | ![image-20251028131012451](assets\image-20251028131012451.png) |
...
|                            仅光照                            |                          漫反射IBL                           |
| :----------------------------------------------------------: | :----------------------------------------------------------: |
| ![image-20251028161859120](assets\image-20251028161859120.png) | ![image-20251028161838215](assets\image-20251028161838215.png) |
|                      漫反射IBL+镜面IBL                       |                         换个HDR贴图                          |
| ![image-20251028161821944](assets\image-20251028161821944.png) | ![image-20251028162107133](assets\image-20251028162107133.png) |
...
这里没有建立单独的buff类，而是只做了buff枚举，并且逻辑是在game类里面直接操作的

这里没啥好说的，就是每帧更新，处理逻辑，和buff逻辑差不多

#### 音效

这也没啥好说的，就是调用一个外部库，直接播放，也不需要做音频管理
...
在涉及透明物体渲染时，需要我们确定好渲染顺序（也就是按深度由远到近），才能保证blend结果是正确的

OIT本身是一个概念而非技术，全称为`Order-indendent transparency`，也就是**不需要我们按顺序绘制对象，也能正确绘制半透明物体**的技术

#### Exact OIT

所谓准确型OIT，就是不论渲染顺序是什么样的，半透明结果一定是和按照正确渲染顺序一样的，严格按照由远及近的顺序进行混合的

##### 流程

这里引入**A-Buffer**的概念，和G-Buffer类似，他也是存储了每个像素的信息，不同的是A-Buffer记录的是多个片元的结果，先一个Pass绘制A-Buffer，再一个Pass绘制混合结果，所以这里的blend操作是被搬到了frag着色器中进行实现

关于A-Buffer的构建，可以用固定大小三维数组存储，但空间浪费较多，也可以用链表来存，额外存一个头指针纹理

##### 思考

其本质上还是依据深度进行排序，由远到近，不过传统cpu排序，是将所有物体进行一次排序，再进行n次draw call，OIT是进行n次draw call（应该也能和G-Buffer一块存）后，在着色器中排序时，每个像素也是并行跑的，且每个像素的片元数量往往不多

考虑过如果用链表也许可以在绘制A-Buffer时就插入排序，但尽管物体时挨个渲染的，但片元着色器的执行是并行的；GPU的流水线式**乱序执行、有序写入**的，MRT天然互斥无所谓，但想要使用UAV，就需要使用原子操作保证结果正确，不可以对UAV进行非原子操作（这里是对fragment着色器中使用UAV的限制，compute着色器中如果不需要采样其他位置的数据，可以对自己的位置进行非原子操作）

| 时间片 | vert | rasterizer | frag | ROP  |
| ------ | ---- | ---------- | ---- | ---- |
| T0     | A    |            |      |      |
| T1     | B C  | A          |      |      |
| T2     | C    | B          | A    |      |
...
另外，**第二个blend的pass是可以用compute着色器来做的**，且计算起来应该比fragment着色器流程更快，尤其是大分辨率时，不过简单操作还是fragment着色器做好些，有ROP加速优化，自动做深度测试、混合等

#### Approximate OIT

所谓近似型的OIT，就是并非按照传统由远到近的流程渲染并blend，而是在大多数情况下接近这个结果

##### Dual Depth Peeling

所谓双深度剥离，就是每一次pass将深度最深和最浅的两个片元分别进行blend，记录在front color和back color中，多次pass不断累积，单深度剥离就是只剥离最浅（最影响结果）的那个片元；除了要记录color外，还需要记录上一次的最深/最浅值

它实际上是一个很准确的方法，不过在实际操作中，由于多个pass太耗时间，所以大多只剥离前几层，剩下的用weighted，至于为什么这里仍然使用dual的，是因为和双层的相比，单层的亮度不均、背景偏暗，对weighted的结果有帮助

为什么没有双层以上的？这是由于UAV的限制，由于限制于原子操作（在fragment着色器中），是不能进行排序的，compute着色器又实现不了，因此只能依靠最近和最远这两个来进行比较和替换

##### weighted blended

这个就是真的近似的OIT了，它的思想就是依据深度和透明度贡献一个加权值，权重公式可以自己取
$$
Color = \frac{\sum_i^n c_i*a_i*w_i}{a_i*w_i}
$$
说说过程：首先绘制所有的Opaque物体，然后再在另一个纹理上绘制Transparent物体，得到上述公式的分子和分母（`accum`），以及混合后的alpha值（`revealage`）；接着将这两个纹理结果作为输入，在blend中计算最终颜色，最后靠ROP的混合

这里有几个要注意的点，一个是Opaque的FBO和Transparent的FBO要共用一个深度缓冲（要保证远处半透明物体被不透明物体遮挡）；一个是绘制Transparent需要关闭更新深度缓冲，但要开深度缓冲；

<img src="E:\code\Learn\opengl\assets\image-20251107191527116.png" alt="image-20251107191527116" style="zoom: 67%;" /> 

一些想法：

- 起初我想将blend和transparent的渲染合并，但全部渲染一遍后，才能得到分子和分母的值，因此是不行的；
- 以及，我考虑先渲染transparent，再渲染opaque，
  - 第一次pass对于深度更浅的部分，采用反向blend，
  - 第二次pass对于深度更深的部分，采用深度测试进行覆盖。
  - 但是，我们之前先渲染opaque，就是为了防止深度更深的半透明物体被绘制到前面来，这样做的话，对于多个半透明物体重叠，就没法确定哪些应该在opaque前方了，就无法正确的blend；这个有点像之前dual depth peeling，就是只考虑了最近和最远的两个物体的深度
...
CSM即级联阴影贴图，是用来解决平行光（点光源用万向阴影贴图，聚光灯不需要考虑范围）的长距离的阴影渲染

如果我们渲染一个平行光的阴影贴图，需要让这个平行光的视野包含摄像机整个的视锥体，当我们的视锥体的长度很长时，这个阴影贴图就需要变得很大，或者变得很糊

为了节省空间，同时让近处的阴影贴图的质量较高，我们将视锥体分成若干个小视锥体（头尾相连），每个视锥体对应一个级别的阴影贴图，平行光的视野每次就只需要覆盖这些小视锥体就好了（如下，箭头是光源方向，三部分视锥体各自被一个长方体给包围）

<img src="https://learnopengl.com/img/guest/2021/CSM/frustum_fitting.png" alt="img" style="zoom:50%;" /> 

然后，在渲染时根据距离，选择合适的阴影贴图就好了

#### 关键点

很多地方都和之前的阴影贴图一致，但有一个问题是，我们如何确定包围某个视锥体的范围？如何得到这个对应的光源的`vp`矩阵？

- 首先，我们可以通过逆变换，将视锥体八个角点给变换到世界空间
- 然后，先确定视图矩阵，我们只需要建立一个从光源看向视锥体中心的视图矩阵
- 最后，确定投影矩阵，正交投影矩阵只需要知道view space中的点的x y z的范围即可，也就是`glm::ortho(minX, maxX, minY, maxY, maxZ, minZ)`
...
那么让我们看看哪个ortho，发现怎么Z的范围是反着的？是由于对于正交投影矩阵，它的取值就是规定了x y z的范围，由于`glm`的`lookAt`得到的矩阵也是右手坐标系，变换后摄像机指向的是-Z方向，因此近平面的z值实际上是比远平面的z值更大的（勘误：这里是对于左手坐标系来说，`glm`是右手坐标系，ortho函数会自动乘以-1，所以不用管）

左右手坐标系会**影响视图矩阵、透视投影矩阵**的实现，也会**影响叉乘**的结果

#### 实现

结果上看起来并没有什么不同，不过我们可以输出`cascadeIndex / 4`

<img src="E:\code\Learn\opengl\assets\image-20251112140104487.png" alt="image-20251112140104487" style="zoom:80%;" /> 

可以看到它是对选择的阴影贴图的index是有区分的

<img src="E:\code\Learn\opengl\assets\image-20251112140217254.png" alt="image-20251112140217254" style="zoom:80%;" /> 

以及在`renderDoc`中我们查看不同index的阴影贴图，可以看到不仅范围越来越大，他们之间还有重叠（见上述原理图），并且index越大的区分越模糊（精度越低）

| index=1                                                      | index=2                                                      | index=3                                                      | index=4                                                      |
| ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| ![image-20251112140614210](E:\code\Learn\opengl\assets\image-20251112140614210.png) | ![image-20251112140629804](E:\code\Learn\opengl\assets\image-20251112140629804.png) | ![image-20251112140653809](E:\code\Learn\opengl\assets\image-20251112140653809.png) | ![image-20251112140709963](E:\code\Learn\opengl\assets\image-20251112140709963.png) |

#### bug

其中一个是关于`glm::ortho(xRange.x, xRange.y, yRange.x, yRange.y, -zRange.y, -zRange.x)`

没错，它的z轴应该是这样的，因为`glm`是右手坐标系，他会自动给z轴乘上一个-1，只需要输入离摄像机前的距离就好了，不需要考虑z值是负的

还有一个是ortho投影矩阵应该是根据观察空间中的坐标作为边界，应该**将世界空间坐标乘以不同index对应的光源视图矩阵**的坐标，而非直接用世界空间坐标了

### 场景

#### 场景图

场景图就是用来存储一个场景中各物体的一个结构，类似于文件系统，子物体的变换受父物体影响

这个其实在上面的动画中有所体现，这里额外做了一点优化，就是使用了脏标记，让物体的transform改变后再更新变换矩阵

写起来挺简单的，这里脏标记只用在了自己的变换矩阵计算中，也就是说一个实体的模型矩阵是父亲的变换矩阵乘以自己的变换矩阵

<img src="E:\code\Learn\opengl\assets\image-20251112165747088.png" alt="image-20251112165747088" style="zoom:80%;" /><img src="E:\code\Learn\opengl\assets\image-20251112165842636.png" alt="image-20251112165842636" style="zoom:80%;" />  
