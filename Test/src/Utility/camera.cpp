...
void camera::genPostprocessingFrameBuffer()
{
    glGenFramebuffers(1, &postprocessingFBO);
    glBindFramebuffer(GL_FRAMEBUFFER, postprocessingFBO);

    // 颜色纹理附件
    glGenTextures(1, &postprocessingColorBufferTex);
    glBindTexture(GL_TEXTURE_2D, postprocessingColorBufferTex);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, opengl::window::getWidth(), opengl::window::getHeight(), 0, GL_RGB, GL_UNSIGNED_BYTE, nullptr);

    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);

    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, postprocessingColorBufferTex, 0);
...
void camera::drawPostprocessing(Shader *shader)
{
    glDisable(GL_BLEND);
    ...
    // 复制帧缓冲内容到默认帧缓冲，以便显示到屏幕上
    glBindFramebuffer(GL_READ_FRAMEBUFFER, postprocessingFBO);
    glBindFramebuffer(GL_DRAW_FRAMEBUFFER, screenFBO);
    glBlitFramebuffer(0, 0, opengl::window::getWidth(), opengl::window::getHeight(),
                      0, 0, opengl::window::getWidth(), opengl::window::getHeight(),
